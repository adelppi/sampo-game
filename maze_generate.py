...
class Maze:
    ...
    def make_maze(self, pos_y, pos_x):
        ...
        for i in vector:
            ...
            if (
                ny < 1 or ny >= self.height - 1 or nx < 1 or nx >= self.width - 1
            ):  # 周りの壁を越えていたらスルー
                continue
    ...
    def max_distance(self):
        ...
        while q:  # キューに要素がなくなるまで回す
            ...
            for i in range(4):
                ...
                if (
                    ny < 0 or nx < 0 or ny >= self.height or nx >= self.width
                ):  # 次に行くマスが壁だったらスルー
                    continue

                if (
                    self.array[ny][nx] == 1 or dist[ny][nx] != -1
                ):  # 次に行くマスが壁だったらスルー
                    continue
